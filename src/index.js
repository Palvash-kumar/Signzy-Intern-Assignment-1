...
app.get('/api/workflows/:id', (req, res) => {
  const workflow = configLoader.get(req.params.id);
...
app.post('/api/workflows', (req, res) => {
  try {
    ...
    logger.info(`Workflow saved via API: ${config.id}`);
...
app.delete('/api/workflows/:id', (req, res) => {
  const workflow = configLoader.get(req.params.id);
  if (!workflow) return res.status(404).json({ success: false, error: 'Workflow not found' });
  configLoader.remove(req.params.id);
  res.json({ success: true, message: `Workflow ${req.params.id} deleted` });
});
